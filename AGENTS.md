...
| 样式 | Tailwind CSS 4（`@theme` token）+ 原生 CSS 组件类；`@bay/landing-ui` tokens 已在 `global.css` 顶部 `@import`，品牌色通过 `--lui-accent` 覆盖 |
...
npm install
npm run dev       # 开发服务器
npm run build     # astro build（含 Playwright 截图生成预览图）
...
- **Tailwind CSS 4**：design token 在 `global.css` 的 `@theme { ... }` 块中定义，新增颜色/间距/圆角先检查 `@theme` 是否已有；组件保留自定义 CSS 类（与其他 landing 混合模式一致），新组件可按需用 Tailwind utility。
